"""HappyHorse 1.0 Video Saver — downloads a video URL, saves to disk, returns frames."""
...
class HappyHorseVideoSaver:
    @classmethod
    def INPUT_TYPES(cls):
        return {"required": {
            "video_url": ("STRING", {"multiline": False, "default": ""}),
            "save_subfolder": ("STRING", {"default": "happyhorse"}),
            "filename_prefix": ("STRING", {"default": "happyhorse"}),
        }, "optional": {
    ...
    def run(self, video_url, save_subfolder, filename_prefix,
        ...
        while os.path.exists(fp):
            ...
            fp = os.path.join(out_dir, f"{filename_prefix}_{n:05d}.mp4")
        try:
            print(f"[HappyHorse Saver] Downloading {video_url[:80]}...")
            ...
            print(f"[HappyHorse Saver] Saved {fname} — {count} frames")
    ...
    def _load(self, path, cap, skip, nth):
        try:
            ...
            print(f"[HappyHorse Saver] frame load error: {e}")
            return torch.zeros(1, 64, 64, 3), 1

    def _err(self, msg):
        print(f"[HappyHorse Saver] ERROR: {msg}")
        return {"ui": {"text": [msg]}, "result": (torch.zeros(1, 64, 64, 3), "ERROR", 0)}


NODE_CLASS_MAPPINGS = {"HappyHorseVideoSaver": HappyHorseVideoSaver}
NODE_DISPLAY_NAME_MAPPINGS = {"HappyHorseVideoSaver": "🐎 HappyHorse 1.0 Save Video"}
